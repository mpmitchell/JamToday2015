...
public class GameController : MonoBehaviour {
    ...
    private LinkedList<Pattern> recordedPatterns_ = new LinkedList<Pattern>();
    ...
    public void RegisterAttempt() {
        ...
        if (newPatterns_.Count > 0) {
            ...
            var pattern = recordedPatterns_.First;
            for (int i = 0; pattern != null; ++i, pattern = pattern.Next) {
                GameObject scoreDisaply = Instantiate(scoreDisplayPrefab_) as GameObject;
                scoreDisaply.GetComponent<ScoreDisplay>().Initiate(pattern.Value, i * 4);
                scoreDisaply.transform.parent = scoreContainer_.transform;
            }
        }
    }
}
